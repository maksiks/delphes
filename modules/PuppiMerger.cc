...
void PuppiMerger::Process()
{
  ...
  for(itInputList = fInputList.begin(); itInputList != fInputList.end(); ++itInputList)
  {
    ...
    {
      TLorentzVector tmp = candidate->Momentum;
      tmp *= candidate->puppiW;
      const TLorentzVector &candidateMomentum = tmp;
